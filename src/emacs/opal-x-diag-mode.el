...
(defun opal-diag-clear-errors ()
  ...
  (if (> (length opal-diag-errors) 0)
	...
	(while (>= opal-diag-curr-error 0)
	  (delete-extent (opal-diag-src-of (opal-diag-get-current-diag)))
	  (delete-extent (opal-diag-err-of (opal-diag-get-current-diag)))
	  (setq opal-diag-curr-error (- opal-diag-curr-error 1))
	  )
	)
    )
  ...
  (if opal-diag-buffer
	...
	(cond (opal-diag-hide (switch-to-buffer opal-diag-hide))
	      ...
	      (opal-diag-errors (switch-to-buffer
				  (extent-object (opal-diag-src-of
...
(defun opal-diag-parse-ext-for-unknown ()
  ...
  (setq new-err-ext
	(make-extent err-start err-end
		     (get-buffer opal-diag-buffer)))
  (set-extent-keymap new-err-ext ext-keymap)
  (set-extent-face new-err-ext 'opal-diag-source-error-face)
  (set-extent-mouse-face new-err-ext 'default)
)
...
(defun opal-diag-parse-src-ext ()
  (setq new-src-ext
	(make-extent src-start src-end (get-buffer src)))
  (set-extent-mouse-face new-src-ext 'opal-diag-source-error-face)
  (set-extent-keymap new-src-ext ext-keymap)
  (cond ((equal 'hint (opal-diag-parse-type type))
	 (set-extent-face new-src-ext 'opal-diag-mouse-hint-face))
	((equal 'warning (opal-diag-parse-type type))
	 (set-extent-face new-src-ext 'opal-diag-mouse-warning-face))
	(t
	 (set-extent-face new-src-ext 'opal-diag-mouse-error-face))
	)
  )

(defun opal-diag-parse-err-ext ()
  (setq new-err-ext (make-extent err-start err-end
				 (get-buffer opal-diag-buffer)))
  (cond ((equal 'hint (opal-diag-parse-type type))
	 (set-extent-face new-err-ext 'opal-diag-error-hint-face)
	 (set-extent-mouse-face new-err-ext 'opal-diag-mouse-hint-face))
	 ((equal 'warning (opal-diag-parse-type type))
	  (set-extent-face new-err-ext 'opal-diag-error-warning-face)
	  (set-extent-mouse-face new-err-ext 'opal-diag-mouse-warning-face))
	 (t
	  (set-extent-face new-err-ext 'opal-diag-error-error-face)
	  (set-extent-mouse-face new-err-ext 'opal-diag-mouse-error-face))
	 )
  (set-extent-keymap new-err-ext ext-keymap)
  )
...
(defvar opal-diag-info-buffer "*opal-diag-information $Revision$*"
...
;  (interactive)
  (switch-to-buffer (extent-object err-ext))
  (goto-char (extent-start-position err-ext))
