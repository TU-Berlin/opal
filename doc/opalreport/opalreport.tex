...
\title{{\sc The Programming Language Opal}\\
       {\small 6$^{\mbox{\small th}}$ corrected edition}}
\author{Programming Languages and Compiler Construction Group \\
        Department of Software Engineering and Theoretical Computer Science \\
        School IV -- Electrical Engineering and Computer Science \\
        Technische Universit\"at Berlin\\[1ex]
        {\small Edited by PETER PEPPER and FLORIAN LORENZEN}}
\date{October 2012 \bigskip}
...
Section~6 covers lazy evaluation and a description of how to construct
an {\sc Opal} program rounds off this report in section~7.
...
\begin{tabbing}
  ...
  \nt{Lexeme} \> = \> \nt{Keyword} $\cup$ \nt{Ide} $\cup$ \nt{Denotation}
                      $\cup$ \nt{Separator} $\cup$ \\
              \>   \> \nt{Comment} $\cup$ \nt{Pragma} $\cup$ \nt{Layout} \\
  \nt{Keyword} \> = \> \{ \vb{ALL AND ANDIF AS COMPLETELY DATA DEF DFD ELSE FORCE
    EX FI } \\
               \>   \> \> \vb{FUN IF IMPLEMENTATION IMPORT IN LAW LAZY LET NOT
                 ONLY} \\
               \>   \> \> \vb{OR ORIF OTHERWISE SIGNATURE SORT SUSPEND THEN TYPE WHERE } \\
...
Changes since the 4$^{th}$ edition:
\begin{itemize}
...
Changes since the 5$^{th}$ edition:
\begin{itemize}
\item Support for lazy evaluation using explicit force and suspend has
  been added.
\end{itemize}
